        ...
        ForEach ($c in $computer) {
            ForEach ($p in $port) {
                ...
                If ($udp) {
                  ...
                  if ($udpobject -ne $null) {$connected = $true} else {$connected = $false}
                    ...
                    Catch {
                        ...
                        ElseIf (($Error[0].ToString() -match "forcibly closed by the remote host") -OR (!$connected) ) {
                            #Close connection
                          if ($udpobject -ne $null) {$udpobject.Close()}
                            Write-Verbose "Connection Timeout"
                            #Build report
                            $temp.Server = $c
                            $temp.Port = $p
